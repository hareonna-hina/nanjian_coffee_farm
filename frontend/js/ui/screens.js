...
(function(CG){
  ...
  /** 切换到经营页，加载当前阶段关键图片，并在后台预取本轮事件图与下一阶段资源。 */
  CG.ui.renderGame=function(){const s=CG.stateStore.current,round=CG.rounds[s.round]||CG.rounds[4],gameScreen=document.getElementById("game-screen");document.getElementById("start-screen").classList.remove("is-active");gameScreen.style.backgroundImage="url('assets/images/ui/mainpage_background.png')";gameScreen.classList.add("is-active");document.getElementById("stage-image").src=round.visual;document.getElementById("round-title").textContent=round.title;document.getElementById("round-story").textContent=round.story;CG.ui.renderStatus();CG.ui.renderDecisions(round,CG.app.chooseRound);void CG.imageLoading.preloadForRound(s.round)};
  /** 打开昵称/UID 卡；required=true 时禁止关闭，确保首次进入先完成昵称。 */
  CG.ui.showProfile=function(required=false){const p=CG.profile.getOrCreate();CG.ui.openModal(`<article class="modal-card profile-modal" role="dialog" aria-modal="true" aria-label="庄园主身份">${required?"":`<button class="modal-close" type="button" data-close aria-label="关闭">×</button>`}<h2>庄园主档案</h2><label for="player-name">庄园主昵称</label><input id="player-name" maxlength="16" value="${p.playerName||""}" placeholder="例如：无量山咖农_07"><div class="profile-uid">UID ${p.publicUid}</div><button class="primary-button" type="button" data-save-profile>保存昵称</button></article>`,!required);const input=document.getElementById("player-name");document.querySelector("[data-save-profile]").addEventListener("click",()=>{if(!input.value.trim()){CG.ui.toast("请先输入一个庄园主昵称");input.focus();return}CG.profile.updateName(input.value);CG.ui.closeModal();CG.ui.showStart()})};
